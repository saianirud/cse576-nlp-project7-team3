...
def maskNumber(list1, n):
    indices = random.sample(range(0, len(list1)), n)
    outputlist = []
    i = 0
...
if __name__ == '__main__':
    ...
    parser.add_argument('--output_dir', type=str, default='./models/pretrain', required=False,
                        help='Path to save checkpoint and results.')
    ...
    dataset_train = PretrainDataset(data_dir=args.data_dir, type_path='train', sort_type=args.sort_type,
                                    data_size=args.train_size)

    dataset_val = PretrainDataset(data_dir=args.data_dir, type_path='val', sort_type=args.sort_type,
                                  data_size=args.val_size)
    dataset_test = PretrainDataset(data_dir=args.data_dir, type_path='test', sort_type=args.sort_type,
    ...
    val_dataloader = DataLoader(dataset_val, batch_size=args.val_batch_size, shuffle=False,
                                num_workers=args.num_workers)
    test_dataloader = DataLoader(dataset_test, batch_size=args.val_batch_size, shuffle=False,
                                 num_workers=args.num_workers)
    ...
    model = T5Pretrainer(hparams=args, train_dataloader=train_dataloader, val_dataloader=val_dataloader,
    ...
    model = T5Pretrainer.load_from_checkpoint(checkpoint_path, train_dataloader=train_dataloader,
