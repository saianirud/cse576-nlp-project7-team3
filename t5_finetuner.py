...
if __name__ == '__main__':
    ...
    parser.add_argument('--output_dir', type=str, default='./models/finetune', required=False,
                        help='Path to save checkpoint and results.')
    parser.add_argument('--model_name_or_path', type=str, required=True)
    parser.add_argument("--ckpt_path", default=None, type=str,
                        help="Path to model checkpoint. If nothing specified, new model will be created.")
    ...
    test_dataloader = DataLoader(dataset_test, batch_size=args.val_batch_size, shuffle=False,
                                 num_workers=args.num_workers)
    test_dataloader_ood = DataLoader(dataset_test_ood, batch_size=args.val_batch_size, shuffle=False,
                                     num_workers=args.num_workers)
    ...
    if args.ckpt_path:
        ...
        model = T5Finetuner.load_from_checkpoint(checkpoint_path, train_dataloader=train_dataloader,
                                                 val_dataloader=val_dataloader, test_dataloader=test_dataloader)
    else:
        model = T5Finetuner(hparams=args, train_dataloader=train_dataloader, val_dataloader=val_dataloader,
    ...
    '''
    model = T5Finetuner.load_from_checkpoint(checkpoint_path, train_dataloader=train_dataloader,
                                             val_dataloader=val_dataloader, test_dataloader=test_dataloader_ood)
