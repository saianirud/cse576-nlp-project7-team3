...
def get_data_loaders(path, args, pretrain=True):
    ...
    dataset_train = PretrainDataset(data_train_asc + data_train_desc) if pretrain else FinetuneDataset(data_train_asc + data_train_desc)
    dataset_val = PretrainDataset(data_val_asc + data_val_desc) if pretrain else FinetuneDataset(data_val_asc + data_val_desc)
    dataset_test = PretrainDataset(data_test_asc + data_test_desc) if pretrain else FinetuneDataset(data_test_asc + data_test_desc)
...
class PretrainDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if not example['asc']:
            label = 'No'
            numbers_string = ', '.join(str(x) for x in example['input'])
            random.shuffle(example['output'])
            sorted_numbers_string = ', '.join(str(x) for x in example['output'])
...
if __name__ == '__main__':
    ...
    '''
    train_dataloader, val_dataloader, test_dataloader = get_data_loaders('dataset_pretrain.json', args, True)

    checkpoint_callback = ModelCheckpoint(
        filepath=os.path.join(args.output_dir, 'pretrain-{epoch}-{val_exact_match:.4f}'),
    ...
    model = T5Pretrainer(hparams=args,
    ...
    checkpoint_path = glob.glob(os.path.join(args.output_dir, 'pretrain-*.ckpt'))[0]
    model = T5Pretrainer.load_from_checkpoint(checkpoint_path,
                                             train_dataloader=train_dataloader,
                                             val_dataloader=val_dataloader,
                                             test_dataloader=test_dataloader,
                                             orthography=args.orthography)
    ...
    '''
    train_dataloader, val_dataloader, test_dataloader = get_data_loaders('dataset_finetune.json', args, False)

    checkpoint_callback = ModelCheckpoint(
        filepath=os.path.join(args.output_dir, 'finetune-{epoch}-{val_exact_match:.4f}'),
        verbose=False, save_last=False, save_top_k=1, mode='max', monitor='val_exact_match',
        save_weights_only=False, period=args.check_val_every_n_epoch)
